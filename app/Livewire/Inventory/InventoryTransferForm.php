...
class InventoryTransferForm extends Component
{
    ...
    public bool $referenceAutoGenerated = false;
    ...
    public function mount(?StockMovement $stockMovement = null): void
    {
        ...
        if (!$stockMovement || !$stockMovement->exists) {
            if ($this->saleReference !== '') {
                $this->reference = $this->saleReference;
            } else {
                $this->reference            = 'TRF-' . now()->format('Ymd') . '-' . strtoupper(Str::random(4));
                $this->referenceAutoGenerated = true;
            }
            return;
        }
    ...
    {
        if ($this->stockMovement?->exists) return;
        $this->reference              = 'TRF-' . now()->format('Ymd') . '-' . strtoupper(Str::random(4));
        $this->referenceAutoGenerated = true;
    }
    ...
    public function productsPicked(array $productIds): void
    {
        foreach ($productIds as $productId) {
            $this->addFromCatalog((int) $productId);
        }
    }
        ...
        DB::transaction(function () {
            ...
            foreach ($this->items as $index => $item) {
                ...
                if ($dispatched > 0) {
                    ...
                    // Move FIFO lots: origin → transit
                    app(FifoStockService::class)->moveLotsBetweenWarehouses(
                        companyId:              $companyId,
                        productId:              $item['product_id'],
                        originWarehouseId:      $this->warehouse_id,
                        destinationWarehouseId: $transitWarehouse->id,
                        quantity:               $dispatched,
                        reference:              $this->stockMovement->folio,
                        movementTypeOut:        'transfer_out',
                        movementTypeIn:         'transfer_in',
                        notes:                  'Despacho hacia tránsito',
                    );
                }
            ...
            foreach ($this->stockMovement->items as $dbItem) {
                ...
                if ($qty > 0) {
                    ...
                    app(FifoStockService::class)->moveLotsBetweenWarehouses(
                        ...
                        movementTypeOut:        'transfer_out',
                        movementTypeIn:         'transfer_in',
                        notes:                  'Transferencia local hacia tránsito',
                    );
        ...
        DB::transaction(function () {
            ...
            foreach ($this->items as $item) {
                ...
                if ($recvQty !== null && $recvQty > 0) {
                    ...
                    // Move FIFO lots: transit → destination (preserva lote, agrega T si hay colisión)
                    app(FifoStockService::class)->moveLotsBetweenWarehouses(
                        companyId:              $companyId,
                        productId:              $item['product_id'],
                        originWarehouseId:      $transitWarehouse->id,
                        destinationWarehouseId: $this->warehouse_destination_id,
                        quantity:               $recvQty,
                        reference:              $this->stockMovement->folio,
                        movementTypeOut:        'transfer_out',
                        movementTypeIn:         'transfer_in',
                        notes:                  'Recepción desde ' . ($this->stockMovement->warehouse?->name ?? 'origen'),
                    );
        ...
        if (in_array($this->stockMovement->status, ['accepted', 'in_transit'])) {
            ...
            DB::transaction(function () use ($transitWarehouse) {
                ...
                foreach ($this->stockMovement->items as $dbItem) {
                    ...
                    // Revertir lotes: tránsito → origen
                    app(FifoStockService::class)->moveLotsBetweenWarehouses(
                        companyId:              auth()->user()->company_id,
                        productId:              $dbItem->product_id,
                        originWarehouseId:      $transitWarehouse->id,
                        destinationWarehouseId: $this->warehouse_id,
                        quantity:               $dispatched,
                        reference:              $this->stockMovement->folio,
                        movementTypeOut:        'transfer_out',
                        movementTypeIn:         'transfer_in',
                        notes:                  'Reversión por cancelación de transferencia',
                    );
                }
