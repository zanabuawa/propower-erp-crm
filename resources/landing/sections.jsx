// ProPower — componentes interactivos
import React, { useState, useEffect, useRef } from 'react';

const WHATSAPP = '526141666340';
const PHONE    = '614 166 6340';
const EMAIL    = 'ventas@propower.mx';
...
export const Nav = ({ mobile = false, active = 'Inicio' }) => {
      ...
      <>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', background: navBg, borderBottom:'1px solid rgba(255,255,255,0.08)', position:'sticky', top:0, zIndex:200, backdropFilter:'blur(10px)', transition:'background 0.3s' }}>
          ...
          <button onClick={() => setOpen(o => !o)} style={{ background:'none', border:'none', cursor:'pointer', padding:4, display:'flex', flexDirection:'column', gap:5 }}>
            <div style={{ width:26, height:2, background:'#fff', transition:'transform 0.2s', transform: open ? 'rotate(45deg) translate(5px,5px)' : 'none' }}/>
            <div style={{ width:26, height:2, background:'#fff', opacity: open ? 0 : 1, transition:'opacity 0.2s' }}/>
            <div style={{ width:26, height:2, background:'#fff', transition:'transform 0.2s', transform: open ? 'rotate(-45deg) translate(5px,-5px)' : 'none' }}/>
          </button>
        </div>
        {open && (
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'22px 56px', background: navBg, borderBottom:'1px solid rgba(255,255,255,0.08)', color:'#fff', position:'sticky', top:0, zIndex:200, backdropFilter:'blur(10px)', transition:'background 0.3s, box-shadow 0.3s', boxShadow: scrolled ? '0 2px 20px rgba(0,0,0,0.5)' : 'none' }}>
      <img src={LOGO_H_RED} alt="ProPower" style={{ height:44 }} />
      <div style={{ display:'flex', gap:40 }}>
        {NAV_LINKS.map(({ l, id }) => (
          <button key={l} onClick={() => scrollTo(id)}
            style={{ background:'none', border:'none', cursor:'pointer', padding:0, color: l === active ? 'var(--pp-red)' : '#fff', fontSize:14, fontWeight:500, letterSpacing:'0.02em', fontFamily:'Figtree, sans-serif', position:'relative' }}
          >
            {l}
      ...
      <div style={{ position:'relative', height:720, overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:3, padding:'64px 56px', maxWidth:860 }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:10, fontSize:12, letterSpacing:'0.22em', textTransform:'uppercase', color:'var(--pp-red)', fontFamily:'JetBrains Mono, monospace', marginBottom:24 }}>
            ...
            ProPower Electroconstrucciones · desde 2012
          </div>
          ...
          <div style={{ display:'flex', gap:20, marginTop:32, alignItems:'center' }}>
            <button onClick={() => scrollTo('contacto')} style={{ padding:'16px 30px', background:'var(--pp-red)', color:'#fff', fontSize:14, fontWeight:700, letterSpacing:'0.04em', textTransform:'uppercase', border:'none', cursor:'pointer' }}
              onMouseEnter={e => e.target.style.opacity='0.85'} onMouseLeave={e => e.target.style.opacity='1'}>
              Contáctanos →
            </button>
            <button onClick={() => scrollTo('servicios')} style={{ background:'none', border:'none', cursor:'pointer', fontSize:13, color:'rgba(255,255,255,0.7)', fontFamily:'JetBrains Mono, monospace', letterSpacing:'0.08em', textTransform:'uppercase' }}>
          ...
          <div style={{ display:'flex', gap:6 }}>
            {HERO_IMGS.map((_, i) => (
              <button key={i} onClick={() => setSlide(i)}
                style={{ width: i === slide ? 36 : 10, height:3, background: i === slide ? 'var(--pp-red)' : 'rgba(255,255,255,0.35)', border:'none', cursor:'pointer', padding:0, transition:'width 0.3s, background 0.3s' }}
              />
            ))}
          </div>
    ...
    <div id="inicio" style={{ background:'#0a0a0a', color:'#fff' }}>
      ...
      <div style={{ position:'relative', height:580, overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:20, zIndex:3 }}>
          <div style={{ fontSize:10, letterSpacing:'0.22em', textTransform:'uppercase', color:'var(--pp-red)', fontFamily:'JetBrains Mono, monospace', marginBottom:14 }}>◼ Desde 2012</div>
          ...
          <button onClick={() => scrollTo('contacto')} style={{ display:'block', width:'100%', padding:'14px 22px', background:'var(--pp-red)', color:'#fff', fontSize:13, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.04em', marginTop:18, textAlign:'center', border:'none', cursor:'pointer' }}>
            Contáctanos →
          </button>
        </div>
      </div>
...
export const OfertaDesktop = () => {
  ...
  return (
    <section ref={ref} id="oferta" style={{ padding:'120px 56px', background:'#fff', ...revealStyle(vis) }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1.4fr 1fr 1fr', gap:16 }}>
        {cards.map((c,i) => (
          <div key={c.t} style={{ position:'relative', height:520, overflow:'hidden', cursor:'pointer' }}
            onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} onClick={() => scrollTo('contacto')}>
            <img src={c.img} alt={c.t} style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', transition:'transform 0.6s', transform: hov===i ? 'scale(1.06)' : 'scale(1)' }}/>
            ...
            <div style={{ position:'absolute', top:24, left:24, right:24, display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
              <div style={{ width:56, height:56, border:`1px solid ${hov===i?'var(--pp-red)':'rgba(255,255,255,0.35)'}`, display:'flex', alignItems:'center', justifyContent:'center', transition:'border-color 0.3s' }}>
                <c.Icon/>
              </div>
            ...
            <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:28, color:'#fff' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10, marginTop:22, fontSize:13, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.04em', color:'var(--pp-red)', transition:'transform 0.3s', transform: hov===i ? 'translateX(6px)' : 'none' }}>
                Más información →
              </div>
            </div>
          </div>
...
export const NosotrosDesktop = () => {
  const [ref, vis] = useReveal();
  return (
    <section ref={ref} id="nosotros" style={{ padding:'120px 56px', background:'#0a0a0a', color:'#fff', ...revealStyle(vis) }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1.2fr', gap:80 }}>
        <div>
          ...
          <div style={{ marginTop:40, fontFamily:'Archivo Narrow, sans-serif', fontSize:18, color:'rgba(255,255,255,0.55)', lineHeight:1.6, letterSpacing:'0.02em' }}>Desde 2012 · Chihuahua · México</div>
        </div>
        <div>
...
export const ServiciosDesktop = () => {
  ...
  const [hov, setHov] = useState(null);
  return (
    <section ref={ref} id="servicios" style={{ padding:'120px 56px', background:'#fff', ...revealStyle(vis) }}>
      ...
      <div style={{ border:'1px solid #0a0a0a' }}>
        {SERVICES.map((s,i) => (
          <div key={s.n}
            style={{ display:'grid', gridTemplateColumns:'80px 1fr 2fr 120px', gap:24, alignItems:'center', padding:'26px 28px', borderBottom: i<SERVICES.length-1?'1px solid #0a0a0a':'none', background: hov===i?'#0a0a0a':'#fff', transition:'background 0.25s', cursor:'pointer' }}
            onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} onClick={() => scrollTo('contacto')}>
            <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:14, color:'var(--pp-red)', letterSpacing:'0.1em' }}>{s.n} /</div>
            <div style={{ fontFamily:'Archivo, sans-serif', fontSize:26, fontWeight:700, letterSpacing:'-0.02em', textTransform:'uppercase', color: hov===i?'#fff':'#0a0a0a', transition:'color 0.25s' }}>{s.t}</div>
            <div style={{ fontSize:15, color: hov===i?'rgba(255,255,255,0.7)':'#44403c', lineHeight:1.5, transition:'color 0.25s' }}>{s.d}</div>
            <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:12, color: hov===i?'var(--pp-red)':'#0a0a0a', textTransform:'uppercase', letterSpacing:'0.1em', textAlign:'right', transition:'color 0.25s, transform 0.25s', transform: hov===i?'translateX(4px)':'none' }}>Saber más →</div>
          </div>
        ))}
      </div>
...
export const GaleriaDesktop = () => {
  ...
  const thumb = (p, key, extraStyle={}) => (
    <div key={key} style={{ position:'relative', overflow:'hidden', cursor:'pointer', ...extraStyle }}
      onMouseEnter={() => setHov(key)} onMouseLeave={() => setHov(null)}>
      <img src={p.img} alt="" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', transition:'transform 0.6s', transform: hov===key?'scale(1.06)':'scale(1)' }}/>
  ...
  return (
    <section ref={ref} id="galeria" style={{ padding:'120px 56px', background:'#fafaf9', ...revealStyle(vis) }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:56 }}>
        <div>
        ...
        <div style={{ display:'flex', gap:8 }}>
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding:'8px 14px', border:'1px solid #0a0a0a', background: filter===f?'#0a0a0a':'transparent', color: filter===f?'#fff':'#0a0a0a', cursor:'pointer', fontSize:11, fontFamily:'JetBrains Mono, monospace', textTransform:'uppercase', letterSpacing:'0.12em', transition:'background 0.2s, color 0.2s' }}
            >{f}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gridTemplateRows:'repeat(3,240px)', gap:12 }}>
        ...
        <div style={{ gridColumn:'1/2', gridRow:'1/3', position:'relative', overflow:'hidden', cursor:'pointer' }}
          onMouseEnter={() => setHov('feat')} onMouseLeave={() => setHov(null)}>
          <img src={feat.img} alt="" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', transition:'transform 0.6s', transform: hov==='feat'?'scale(1.06)':'scale(1)' }}/>
        ...
        <div style={{ gridColumn:'1/3', position:'relative', overflow:'hidden', cursor:'pointer' }}
          onMouseEnter={() => setHov('b1')} onMouseLeave={() => setHov(null)}>
          <img src={PROJECTS[5].img} alt="" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', transition:'transform 0.6s', transform: hov==='b1'?'scale(1.06)':'scale(1)' }}/>
      ...
      <div style={{ display:'flex', justifyContent:'center', marginTop:48 }}>
        <button onClick={() => scrollTo('contacto')} style={{ padding:'16px 36px', background:'#0a0a0a', color:'#fff', fontSize:13, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', fontFamily:'JetBrains Mono, monospace', border:'none', cursor:'pointer' }}
...
export const ContactoDesktop = () => {
  ...
  const inp = { width:'100%', background:'transparent', border:'none', borderBottom:'1px solid rgba(255,255,255,0.15)', color:'#fff', fontSize:17, padding:'18px 0', outline:'none', fontFamily:'Figtree, sans-serif', boxSizing:'border-box' };
  ...
  return (
    <section ref={ref} id="contacto" style={{ background:'#0a0a0a', color:'#fff', ...revealStyle(vis) }}>
            ...
            <form onSubmit={submit}>
              {[{name:'nombre',l:'Nombre',p:'Tu nombre completo',type:'text'},{name:'empresa',l:'Empresa',p:'Nombre de tu empresa',type:'text'},{name:'correo',l:'Correo',p:'correo@empresa.com',type:'email'},{name:'telefono',l:'Teléfono',p:'+52 614 000 0000',type:'tel'}].map(f => (
                <div key={f.name}>
                  ...
                  <input name={f.name} type={f.type} placeholder={f.p} value={form[f.name]} onChange={ch} required={f.name!=='telefono'} style={inp}/>
                </div>
              ))}
              ...
              <div>
                ...
                <textarea name="mensaje" placeholder="Cuéntanos sobre tu obra, ubicación y fechas tentativas…" value={form.mensaje} onChange={ch} rows={3} style={{ ...inp, resize:'none', paddingTop:12 }}/>
              </div>
              <button type="submit" style={{ display:'inline-flex', padding:'18px 36px', background:'var(--pp-red)', color:'#fff', fontSize:14, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em', marginTop:24, border:'none', cursor:'pointer' }}
                onMouseEnter={e => e.target.style.opacity='0.85'} onMouseLeave={e => e.target.style.opacity='1'}>
                Enviar mensaje →
              </button>
            </form>
          ...
          <div style={{ display:'grid', gap:20, marginBottom:48 }}>
            {[
              ...
              <div key={x.k} style={{ display:'flex', justifyContent:'space-between', borderBottom:'1px solid rgba(255,255,255,0.1)', paddingBottom:14 }}>
                ...
                {x.href
                  ? <a href={x.href} style={{ fontFamily:'Archivo, sans-serif', fontSize:22, fontWeight:700, color:'#fff', textDecoration:'none' }} onMouseEnter={e=>e.target.style.color='var(--pp-red)'} onMouseLeave={e=>e.target.style.color='#fff'}>{x.v}</a>
            ...
            {[
              ...
              <div key={s.t} style={{ border:'1px solid rgba(255,255,255,0.15)' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'14px 18px', borderBottom:'1px solid rgba(255,255,255,0.15)' }}>
                  ...
                  <a href={`https://maps.google.com/?q=ProPower+Electroconstrucciones+${s.t}`} target="_blank" rel="noreferrer"
...
export const Footer = () => (
  <footer style={{ background:'#000', color:'#fff', padding:'80px 56px 32px' }}>
    <div style={{ borderBottom:'1px solid rgba(255,255,255,0.15)', paddingBottom:40, marginBottom:40 }}>
      <div style={{ fontFamily:'Archivo, sans-serif', fontSize:160, lineHeight:0.85, fontWeight:900, letterSpacing:'-0.05em', textTransform:'uppercase' }}>
        Pro<span style={{ color:'var(--pp-red)' }}>Power</span><span style={{ color:'var(--pp-red)' }}>.</span>
      </div>
      <div style={{ marginTop:12, fontFamily:'Archivo Narrow, sans-serif', fontSize:16, textTransform:'uppercase', letterSpacing:'0.08em', color:'rgba(255,255,255,0.65)' }}>
    ...
    <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1fr', gap:60, paddingBottom:48 }}>
      <div style={{ fontSize:14, color:'rgba(255,255,255,0.6)', lineHeight:1.6, maxWidth:320 }}>
        Empresa 100% mexicana especializada en servicios electromecánicos industriales y comerciales desde 2012.
      </div>
      {[
        ...
        <div key={col.t}>
          ...
          {col.l.map(x => x.fn
            ? <button key={x.txt} onClick={x.fn} style={{ display:'block', fontSize:14, color:'rgba(255,255,255,0.85)', marginBottom:10, background:'none', border:'none', cursor:'pointer', padding:0, fontFamily:'Figtree, sans-serif' }} onMouseEnter={e=>e.target.style.color='var(--pp-red)'} onMouseLeave={e=>e.target.style.color='rgba(255,255,255,0.85)'}>{x.txt}</button>
    ...
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', borderTop:'1px solid rgba(255,255,255,0.1)', paddingTop:24, fontSize:12, color:'rgba(255,255,255,0.45)' }}>
      ...
      <div style={{ display:'flex', gap:20 }}>
        <a href={`https://wa.me/${WHATSAPP}`} target="_blank" rel="noreferrer" style={{ color:'rgba(255,255,255,0.45)', textDecoration:'none' }} onMouseEnter={e=>e.target.style.color='#25d366'} onMouseLeave={e=>e.target.style.color='rgba(255,255,255,0.45)'}>WhatsApp</a>
        <span style={{ cursor:'pointer' }} onMouseEnter={e=>e.target.style.color='#fff'} onMouseLeave={e=>e.target.style.color='rgba(255,255,255,0.45)'}>Facebook</span>
        <span style={{ cursor:'pointer' }} onMouseEnter={e=>e.target.style.color='#fff'} onMouseLeave={e=>e.target.style.color='rgba(255,255,255,0.45)'}>Instagram</span>
      </div>
    </div>
  </footer>
);
...
export const NosotrosMobile = () => {
  const [ref, vis] = useReveal();
  return (
    <section ref={ref} id="nosotros" style={{ padding:'56px 20px', background:'#0a0a0a', color:'#fff', ...revealStyle(vis) }}>
...
export const ServiciosMobile = () => {
  const [ref, vis] = useReveal();
  return (
    <section ref={ref} id="servicios" style={{ padding:'56px 20px', background:'#fff', ...revealStyle(vis) }}>
...
export const GaleriaMobile = () => {
  const [ref, vis] = useReveal();
  return (
    <section ref={ref} id="galeria" style={{ padding:'56px 20px', background:'#fafaf9', ...revealStyle(vis) }}>
...
export const ContactoMobile = () => {
  ...
  const inp = { width:'100%', background:'transparent', border:'none', borderBottom:'1px solid rgba(255,255,255,0.15)', color:'#fff', fontSize:15, padding:'14px 0', outline:'none', fontFamily:'Figtree, sans-serif', boxSizing:'border-box' };
  return (
    <section ref={ref} id="contacto" style={{ background:'#0a0a0a', color:'#fff', padding:'56px 20px', ...revealStyle(vis) }}>
      ...
      <form onSubmit={submit} style={{ marginTop:20 }}>
        ...
        <div>
          ...
          <textarea name="mensaje" placeholder="Cuéntanos sobre tu proyecto…" value={form.mensaje} onChange={ch} rows={3} style={{ ...inp, resize:'none', paddingTop:10 }}/>
        </div>
        <button type="submit" style={{ display:'block', width:'100%', padding:'14px 24px', background:'var(--pp-red)', color:'#fff', fontSize:13, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em', marginTop:20, textAlign:'center', border:'none', cursor:'pointer' }}>
          Enviar por WhatsApp →
        </button>
      </form>
...
export const FooterMobile = () => (
  <footer style={{ background:'#000', color:'#fff', padding:'40px 20px 24px' }}>
    ...
    <div style={{ borderTop:'1px solid rgba(255,255,255,0.1)', marginTop:24, paddingTop:18, fontSize:11, color:'rgba(255,255,255,0.45)' }}>© 2026 ProPower · 100% mexicana desde 2012</div>
  </footer>
);

// ── WhatsApp flotante (fixed) ─────────────────────────────────────────────────
export const WhatsappFloat = ({ scale = 1 }) => (
  <a href={`https://wa.me/${WHATSAPP}`} target="_blank" rel="noreferrer"
    style={{ position:'fixed', bottom:24, right:24, width:56*scale, height:56*scale, borderRadius:'50%', background:'#25d366', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 8px 24px rgba(37,211,102,0.5)', cursor:'pointer', zIndex:1000, textDecoration:'none', transition:'transform 0.2s, box-shadow 0.2s' }}
    onMouseEnter={e=>{e.currentTarget.style.transform='scale(1.12)';e.currentTarget.style.boxShadow='0 12px 32px rgba(37,211,102,0.7)';}}
    onMouseLeave={e=>{e.currentTarget.style.transform='scale(1)';e.currentTarget.style.boxShadow='0 8px 24px rgba(37,211,102,0.5)';}}
  >
