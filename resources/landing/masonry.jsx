...
// Masonry grid adaptativo — distribuye imágenes en N columnas por altura mínima.
const MasonryGrid = ({ images, columns = 4, gap = 12, onImageClick }) => {
  ...
  React.useEffect(() => {
    images.forEach(item => {
      ...
      const im = new Image();
      im.onload = () => setImgMeta(m => ({ ...m, [src]: { w: im.naturalWidth, h: im.naturalHeight } }));
      im.onerror = () => setImgMeta(m => ({ ...m, [src]: { w: 1, h: 1 } }));
      im.src = src;
    });
  }, [images]);
