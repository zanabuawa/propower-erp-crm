...
<head>
    ...
    <title>Acceso denegado — {{ $company?->name ?? config('app.name') }}</title>
    @if($company?->icon)
...
<body class="bg-[#0f1120] text-white antialiased min-h-screen flex flex-col items-center justify-center p-6"
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        <div class="absolute -top-40 -right-40 w-96 h-96 bg-red-700/10 rounded-full blur-3xl"></div>
        <div class="absolute -bottom-40 -left-40 w-96 h-96 bg-indigo-800/10 rounded-full blur-3xl"></div>
    </div>
    ...
    <div class="relative w-full max-w-md text-center">
        ...
        {{-- Número --}}
        <p class="text-[8rem] font-extrabold leading-none tracking-tight text-transparent bg-clip-text bg-gradient-to-br from-red-400 to-red-600 mb-2 select-none">
            403
        </p>

        {{-- Ícono + texto --}}
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-white mb-3">Acceso denegado</h1>
            <p class="text-white/45 text-sm leading-relaxed">
                No tienes permiso para ver esta página.<br>
                Si crees que es un error, contacta al administrador.
            </p>

            @if(!empty($exception) && app()->environment('local') && $exception->getMessage())
                <p class="mt-4 text-xs text-red-400/80 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 inline-block">
