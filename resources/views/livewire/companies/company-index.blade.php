            ...
            <table class="w-full text-sm min-w-[480px]">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50">
                        ...
                        <th class="text-left px-5 py-3 text-xs font-medium text-gray-500 hidden md:table-cell">Sucursales</th>
                        <th class="text-left px-5 py-3 text-xs font-medium text-gray-500 hidden md:table-cell">Usuarios</th>
                ...
                <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-3">
                                    @if($company->icon)
                                        <img src="{{ Storage::url($company->icon) }}" class="w-8 h-8 rounded-lg object-contain flex-shrink-0">
                                    @else
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 sm:hidden">{{ $company->rfc ?? '—' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-3 text-gray-600 hidden sm:table-cell">{{ $company->rfc ?? '—' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-10 text-center text-gray-400 text-sm">No se encontraron empresas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($confirmingDelete)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-xl border border-gray-200 p-6 w-full max-w-sm">
                ...
                <p class="text-sm text-gray-500 mb-5">Esta acción no se puede deshacer. Se eliminarán también sus sucursales.</p>
