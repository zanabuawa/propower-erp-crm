        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50">
                    ...
                    <th class="text-left px-5 py-3 text-xs font-medium text-gray-500">Válida hasta</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($quotations as $quotation)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-5 py-3 font-mono text-xs font-medium text-gray-900">{{ $quotation->folio }}</td>
                        <td class="px-5 py-3 text-gray-700">{{ $quotation->customer->name }}</td>
                        <td class="px-5 py-3 text-gray-600">{{ $quotation->items_count }}</td>
                        <td class="px-5 py-3 font-medium text-gray-900">
                            {{ $quotation->currency }} ${{ number_format($quotation->total, 2) }}
                        </td>
                        <td class="px-5 py-3 text-gray-600">{{ $quotation->valid_until?->format('d/m/Y') ?? '—' }}</td>
                        <td class="px-5 py-3">
