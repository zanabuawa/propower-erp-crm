<div>
    <x-page-header title="Cotizaciones" description="Gestiona tus cotizaciones y órdenes de venta">
        <x-slot:actions>
            ...
            <table class="w-full text-sm min-w-[580px]">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        ...
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide hidden sm:table-cell">Válida hasta</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($quotations as $quotation)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-5 py-3">
                                ...
                                <p class="text-xs text-gray-400 sm:hidden mt-0.5">{{ $quotation->customer->name }}</p>
                            </td>
                            ...
                            <td class="px-5 py-3 text-gray-500 text-xs hidden sm:table-cell">{{ $quotation->valid_until?->format('d/m/Y') ?? '—' }}</td>
                            <td class="px-5 py-3">
                                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium
                                    {{ \App\Models\SaleQuotation::STATUS_COLORS[$quotation->status] ?? 'bg-gray-100 text-gray-500' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="7"><x-empty-state message="No se encontraron cotizaciones." /></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
