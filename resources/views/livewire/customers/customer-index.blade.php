<div>
    <x-page-header title="Clientes" description="Gestiona tu cartera de clientes">
        <x-slot:actions>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm min-w-[580px]">
                ...
                <tbody class="divide-y divide-gray-100">
                                ...
                                <div class="flex items-center gap-3">
                                    @if($customer->image)
                                        <img src="{{ Storage::url($customer->image) }}"
                                            ...
                                            alt="{{ $customer->name }}">
                                    @else
                                        <div class="w-9 h-9 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-semibold text-xs flex-shrink-0"
                                            aria-hidden="true">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 sm:hidden font-mono mt-0.5">{{ $customer->rfc ?? '—' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-3 font-mono text-xs text-gray-600 hidden sm:table-cell">{{ $customer->rfc ?? '—' }}</td>
                            <td class="px-5 py-3 text-gray-600 hidden md:table-cell">{{ $customer->primary_phone ?? '—' }}</td>
                            <td class="px-5 py-3 text-gray-600 hidden md:table-cell">{{ $customer->contacts_count }}</td>
                            <td class="px-5 py-3">
                                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium {{ $statusColors[$customer->status] ?? '' }}">
                                    {{ \App\Models\Customer::STATUS[$customer->status] ?? $customer->status }}
                                </span>
                            </td>
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition">
                                    <a wire:navigate href="{{ route('contacts.show', $customer) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="6"><x-empty-state message="No se encontraron clientes." /></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
