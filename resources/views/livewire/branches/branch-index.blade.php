<div>
    <x-page-header title="Sucursales" description="Gestiona las sucursales por empresa">
        <x-slot:actions>
            ...
            <table class="w-full text-sm min-w-[520px]">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50">
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide">Sucursal</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide hidden sm:table-cell">Empresa</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide hidden md:table-cell">Código</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide hidden md:table-cell">Ciudad</th>
                        <th class="text-left px-5 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide hidden sm:table-cell">Usuarios</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($branches as $branch)
                        <tr class="hover:bg-gray-50 transition group">
                            <td class="px-5 py-3">
                                ...
                                <p class="text-xs text-gray-400 sm:hidden mt-0.5">{{ $branch->company?->name ?? '—' }}</p>
                            </td>
                            <td class="px-5 py-3 text-gray-600 hidden sm:table-cell">{{ $branch->company?->name ?? '—' }}</td>
                            <td class="px-5 py-3 font-mono text-xs text-gray-500 hidden md:table-cell">{{ $branch->code ?? '—' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7"><x-empty-state message="No se encontraron sucursales." /></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
