<div class="min-h-screen bg-slate-50/50 -m-4 lg:-m-6">
        ...
        <div class="flex items-center justify-between gap-4 max-w-full mx-auto">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider">
                        {{ $branch?->exists ? $name : 'Registro de unidad de negocio operativa' }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                <button type="button" wire:click="save"
                    ...
                    <span>{{ $branch?->exists ? 'Guardar cambios' : 'Crear sucursal' }}</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
        <form wire:submit="save" class="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8">
            ...
            {{-- ── COLUMNA IZQUIERDA: Datos Generales (7 cols) ────────────── --}}
            <div class="md:col-span-7 space-y-6 lg:space-y-8">
                ...
                <div class="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
                    <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/30">
                        <h3 class="text-xs font-bold text-slate-500 uppercase tracking-widest">Configuración de Sucursal</h3>
                    </div>
                    <div class="p-6 lg:p-8 space-y-6">
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1">Empresa Perteneciente *</label>
                            ...
                            @error('company_id') <p class="text-[10px] font-bold text-red-500 uppercase tracking-wider ml-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            <div class="sm:col-span-2 space-y-2">
                                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1">Nombre de Sucursal *</label>
                                <input wire:model="name" type="text" placeholder="Ej. Sucursal Monterrey"
                            ...
                            <div class="space-y-2">
                                ...
                                <input wire:model="code" type="text" maxlength="20" placeholder="MTY-01"
                ...
                <div class="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
                    <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/30">
                        <h3 class="text-xs font-bold text-slate-500 uppercase tracking-widest">Ubicación Física</h3>
                    </div>
                    <div class="p-6 lg:p-8 space-y-6">
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1">Calle y Número</label>
                            <input wire:model="address" type="text" placeholder="Ej. Calle 5 de Mayo #450"
            ...
            {{-- ── COLUMNA DERECHA: Contacto y Estatus (5 cols) ────────────── --}}
            <div class="md:col-span-5 space-y-6 lg:space-y-8">
                
                {{-- Card: Contacto --}}
                <div class="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
                    <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/30">
                        <h3 class="text-xs font-bold text-slate-500 uppercase tracking-widest">Contacto Directo</h3>
                    </div>
                    <div class="p-6 lg:p-8 space-y-6">
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1">Correo de la Sucursal</label>
                            <input wire:model="email" type="email" placeholder="mty@empresa.com"
                                class="w-full px-4 py-3 rounded-2xl border-slate-200 bg-slate-50/30 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 transition-all duration-200">
                    ...
                    <div class="p-6 lg:p-8 space-y-6">
                        <div class="flex items-center justify-between p-4 rounded-2xl bg-emerald-50/50 border border-emerald-100/50">
                            <div>
                                ...
                                <p class="text-[10px] text-emerald-600 uppercase font-bold tracking-wider">¿Sucursal Activa?</p>
                            </div>
                        ...
                        <p class="text-[10px] text-slate-400 px-2 leading-relaxed italic text-center">Las sucursales inactivas no estarán disponibles para asignación de personal ni inventarios.</p>
                    </div>
                </div>
