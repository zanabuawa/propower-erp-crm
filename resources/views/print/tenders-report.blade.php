...
<head>
    ...
    <title>Reporte de Licitaciones – {{ $company->name }}</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .badge-prospecto    { background: #f3f4f6; color: #374151; }
        .badge-en_proceso   { background: #dbeafe; color: #1d4ed8; }
        .badge-presentada   { background: #fef3c7; color: #92400e; }
...
<body>
    ...
    <div class="filters">
        ...
        <div class="filter-chip">Licitaciones: <span>{{ $tenders->count() }}</span></div>
    </div>

    <div class="kpis">
        <div class="kpi">
            <div class="kpi-label">Adjudicadas</div>
            <div class="kpi-value green">{{ $adjudicadasCount }}</div>
        </div>
        <div class="kpi">
            <div class="kpi-label">Presupuesto estimado</div>
            <div class="kpi-value">${{ number_format($totalEstimado, 2) }}</div>
        </div>
        <div class="kpi">
            <div class="kpi-label">Monto adjudicado</div>
            <div class="kpi-value accent">${{ number_format($totalAdjudicado, 2) }}</div>
        </div>
        <div class="kpi">
            <div class="kpi-label">Tasa de éxito</div>
            <div class="kpi-value {{ $tasaExito >= 50 ? 'green' : 'amber' }}">{{ $tasaExito }}%</div>
        </div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Responsable</th>
                <th class="right">Pres. estimado</th>
                <th class="right">Adjudicado</th>
                <th>Fecha entrega</th>
        ...
        <tbody>
            ...
            @forelse($tenders as $tender)
                ...
                <tr>
                    ...
                    <td class="mono bold">{{ $tender->folio }}</td>
                    <td class="bold">{{ $tender->name }}</td>
                    <td>{{ \App\Models\Tender::TYPES[$tender->type] ?? $tender->type }}</td>
                    <td>{{ $tender->customer?->name ?? '—' }}</td>
                    <td>{{ $tender->responsibleUser?->name ?? '—' }}</td>
                    <td class="amount">${{ number_format($tender->estimated_budget ?? 0, 2) }}</td>
                    <td class="amount" style="color:#059669;">${{ number_format($tender->awarded_amount ?? 0, 2) }}</td>
                    <td>{{ $tender->submission_date?->format('d/m/Y') ?? '—' }}</td>
                    <td class="center">
                        <span class="badge badge-{{ $tender->status }}">
                            {{ \App\Models\Tender::STATUSES[$tender->status] ?? $tender->status }}
                        </span>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="10" style="text-align:center;padding:20px;color:#9ca3af;">No hay licitaciones con los filtros seleccionados.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr>
                ...
                <td class="right" style="font-size:13px;color:#ef4444;">${{ number_format($totalEstimado, 2) }}</td>
                <td class="right" style="font-size:13px;color:#059669;">${{ number_format($totalAdjudicado, 2) }}</td>
                <td colspan="2"></td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="footer">
        <span>ProPower ERP • Módulo Licitaciones</span>
