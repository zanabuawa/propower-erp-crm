...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .chip span { font-weight: 600; color: #4f46e5; }
        ...
        /* ── TOTALES ────────────────────────────────────────────── */
        tfoot tr { background:#f0f0ff; border-top: 2px solid #4f46e5; }
        tfoot td { padding: 7px 10px; font-weight: 700; color: #1a1a2e; }
...
<body>
    ...
    {{-- Filtros activos --}}
    <div class="filters">
        <div class="chip">Estado: <span>{{ $statusLabel }}</span></div>
        <div class="chip">Total órdenes: <span>{{ $orders->count() }}</span></div>
        @if($totalMXN > 0)
            <div class="chip">Total MXN: <span>${{ number_format($totalMXN, 2) }}</span></div>
        @endif
        @if($totalUSD > 0)
            <div class="chip">Total USD: <span>{{ number_format($totalUSD, 2) }}</span></div>
        @endif
    </div>
    ...
    <div class="kpis">
        <div class="kpi">
            ...
            <div class="kpi-value indigo">{{ $orders->count() }}</div>
        </div>
        <div class="kpi">
        ...
        <div class="kpi">
            ...
            <div class="kpi-value indigo">${{ number_format($totalMXN, 2) }}</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            @forelse($orders as $order)
                ...
                <tr>
                    ...
                    <td class="amount" style="color:#4f46e5;">${{ number_format($order->total, 2) }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <td class="right" style="color:#4f46e5;font-size:13px;">
