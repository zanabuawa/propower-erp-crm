...
<head>
    ...
    <title>Reporte de Asistencias – {{ $company->name }}</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .kpi-label { font-size: 8px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: #9ca3af; margin-bottom: 3px; }
        ...
        .badge-half_day { background: #dbeafe; color: #1d4ed8; }
        .badge-holiday  { background: #f3f4f6; color: #374151; }
        .badge-rest_day { background: #f0f9ff; color: #0369a1; }
...
<body>
    ...
    <div class="header">
        <div style="display:flex; align-items:center; gap:14px;">
            ...
            <div class="header-left">
                ...
                <p>{{ $filterEmployee ? $filterEmployee->first_name . ' ' . $filterEmployee->last_name : $company->name }}</p>
            </div>
        </div>
    ...
    <div class="filters">
        ...
        @if($filterEmployee)
            <div class="filter-chip">Empleado: <span>{{ $filterEmployee->first_name }} {{ $filterEmployee->last_name }}</span></div>
        @endif
        <div class="filter-chip">Registros: <span>{{ $attendances->count() }}</span></div>
        <div class="filter-chip">Horas trabajadas: <span>{{ number_format($totalHours, 1) }} h</span></div>
        @if($totalOvertime > 0)
            <div class="filter-chip">Horas extra: <span>{{ number_format($totalOvertime, 1) }} h</span></div>
        @endif
    </div>
    ...
    <div class="kpis">
        ...
        <div class="kpi">
            ...
            <div class="kpi-value {{ $key === 'present' ? 'green' : ($key === 'absent' ? 'red' : ($key === 'late' ? 'amber' : '')) }}">
                {{ $data['count'] }}
            </div>
        </div>
        @endforeach
        ...
        <tbody>
            ...
            @forelse($attendances as $att)
                ...
                <tr>
                    ...
                    <td class="bold">{{ $att->employee?->last_name }} {{ $att->employee?->first_name }}</td>
                    <td>{{ $att->date?->format('d/m/Y') }}</td>
                    <td class="center mono">{{ $att->check_in ? \Carbon\Carbon::parse($att->check_in)->format('H:i') : '—' }}</td>
                    <td class="center mono">{{ $att->check_out ? \Carbon\Carbon::parse($att->check_out)->format('H:i') : '—' }}</td>
                    <td class="right">{{ $att->worked_hours > 0 ? number_format($att->worked_hours, 1) . ' h' : '—' }}</td>
                    <td class="right" style="color:{{ $att->overtime_hours > 0 ? '#d97706' : '#9ca3af' }}">
                        {{ $att->overtime_hours > 0 ? number_format($att->overtime_hours, 1) . ' h' : '—' }}
                    </td>
                    <td class="center">
                        <span class="badge badge-{{ $att->status }}">
                            {{ \App\Models\HrAttendance::STATUSES[$att->status] ?? $att->status }}
                        </span>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" style="text-align:center;padding:20px;color:#9ca3af;">No hay registros con los filtros seleccionados.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr>
                ...
                <td class="right" style="font-size:13px;color:#ef4444;">{{ number_format($totalHours, 1) }} h</td>
                <td class="right" style="font-size:13px;color:#d97706;">{{ number_format($totalOvertime, 1) }} h</td>
                <td></td>
            </tr>
        </tfoot>
    </table>
