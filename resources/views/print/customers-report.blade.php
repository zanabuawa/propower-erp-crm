...
<head>
    ...
    <title>Directorio de Clientes – {{ $company->name }}</title>
    <style>
...
<body>
    ...
    <div class="filters">
        ...
        <div class="filter-chip">Segmento: <span>{{ $segmentLabel }}</span></div>
        <div class="filter-chip">Total clientes: <span>{{ $customers->count() }}</span></div>
    </div>
    ...
    <div class="kpis">
        ...
        <div class="kpi">
            ...
            <div class="kpi-value amber">{{ $prospectCount }}</div>
        </div>
        <div class="kpi">
        ...
        <tbody>
            ...
            @forelse($customers as $customer)
                ...
                <tr>
                    ...
                    <td class="mono">{{ $customer->rfc ?? '—' }}</td>
                    <td>{{ $customer->phones?->first()?->number ?? '—' }}</td>
                    <td>{{ $customer->emails?->first()?->email ?? '—' }}</td>
                    <td>{{ $customer->segment ? strtoupper($customer->segment) : '—' }}</td>
            ...
            @empty
                <tr>
                    <td colspan="8" style="text-align:center;padding:20px;color:#9ca3af;">No hay clientes con los filtros seleccionados.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr>
                ...
                <td colspan="6" style="font-size:13px;color:#ef4444;">{{ $customers->count() }} clientes</td>
            </tr>
        </tfoot>
    </table>

    <div class="footer">
        <span>ProPower ERP • Módulo CRM</span>
