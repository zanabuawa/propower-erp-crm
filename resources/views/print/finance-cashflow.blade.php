...
<head>
    ...
    <title>Flujo de Caja – {{ $company->name }}</title>
    <style>
...
<body>
    ...
    <div class="filters">
        <div class="filter-chip">Flujo: <span>{{ $flowLabel }}</span></div>
        <div class="filter-chip">Tipo: <span>{{ $typeLabel }}</span></div>
        @if($filterAccount)
            <div class="filter-chip">Cuenta: <span>{{ $filterAccount->name }}</span></div>
        @endif
        <div class="filter-chip">Registros: <span>{{ $cashflows->count() }}</span></div>
    </div>

    <div class="kpis">
        <div class="kpi">
            <div class="kpi-label">Entradas</div>
            <div class="kpi-value green">${{ number_format($totalEntradas, 2) }}</div>
        </div>
        <div class="kpi">
            <div class="kpi-label">Salidas</div>
            <div class="kpi-value red">${{ number_format($totalSalidas, 2) }}</div>
        </div>
        <div class="kpi">
        ...
        <div class="kpi">
            ...
            <div class="kpi-value green">{{ $realizedCount }}</div>
        </div>
    </div>
        ...
        <tbody>
            ...
            @forelse($cashflows as $cf)
                ...
                <tr>
                    ...
                    <td class="bold">{{ $cf->concept ?? $cf->description ?? '—' }}</td>
                    <td>{{ $cf->account?->name ?? '—' }}</td>
                    <td class="center">
                        <span class="badge badge-{{ $cf->flow }}">
                            {{ $cf->flow === 'in' ? 'Entrada' : 'Salida' }}
                        </span>
                    </td>
                    <td>{{ $cf->type ?? '—' }}</td>
                    <td class="amount" style="color:{{ $cf->flow === 'in' ? '#059669' : '#dc2626' }}">
                        ${{ number_format($cf->amount, 2) }}
                    </td>
                    <td>{{ $cf->expected_date?->format('d/m/Y') ?? '—' }}</td>
                    <td>{{ $cf->realized_date?->format('d/m/Y') ?? '—' }}</td>
                    <td class="center">
                        <span class="badge badge-{{ $cf->status }}">
                            {{ \App\Models\FinanceCashflow::STATUSES[$cf->status] ?? $cf->status }}
                        </span>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="9" style="text-align:center;padding:20px;color:#9ca3af;">No hay registros con los filtros seleccionados.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr>
                ...
                <td class="right" style="font-size:13px;color:#ef4444;">${{ number_format($saldoNeto, 2) }}</td>
                <td colspan="3"></td>
            </tr>
        </tfoot>
    </table>
