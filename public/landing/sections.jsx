// ProPower — componentes de secciones completas
// Expuestos en window al final para que ProPower.html los consuma.
    ...
    return (
      <div style={{
        ...
        <a href="#inicio" className="pp-logo-link">
          <img src={LOGO_H_RED} alt="ProPower" style={{ height: 48, display: 'block' }} />
        </a>
...
const HeroDesktop = () => {
  ...
  <div id="inicio" style={{ width: '100%', background: '#0a0a0a', color: '#fff', fontFamily: 'Figtree, sans-serif' }}>
    ...
    <div style={{ position: 'relative', height: 720, overflow: 'hidden' }}>
      ...
      {/* marca de agua removida para mostrar foto limpia */}
...
const HeroMobile = () => (
  <div id="inicio" style={{ background: '#0a0a0a', color: '#fff' }}>
    ...
    <div style={{ position: 'relative', height: 580, overflow: 'hidden' }}>
      <img src={HERO_IMGS[0]} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(10,10,10,0.5) 0%, rgba(10,10,10,0.92) 60%)' }} />
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 0, border: '1px solid rgba(255,255,255,0.15)', marginTop: 28 }}>
        ...
        <div style={{ padding: '14px 22px', background: 'var(--pp-red)', color: '#fff', fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', marginTop: 18, textAlign: 'center' }}>
          Contáctanos →
        </div>
      </div>
    </div>
...
// Compatibilidad con el bloque mobile que aun usa SERVICES
const SERVICES = SERVICES_INDUSTRIA.slice(0, 8).map((s, i) => ({
  n: String(i + 1).padStart(2, '0'), t: s.t, d: 'Personal certificado y experiencia en obra industrial.',
}));
...
const ServiceCard = ({ s, i }) => {
  ...
  return (
    <a href="#contacto" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{
      position: 'relative', display: 'block', height: 280, overflow: 'hidden',
...
const GaleriaDesktop = () => (
  <section id="galeria" style={{ padding: '120px 56px', background: '#fafaf9' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gridTemplateRows: 'repeat(3, 240px)', gap: 12 }}>
      ...
      {PROJECTS.slice(1, 5).map((p) => (
        <div key={p.t} style={{ position: 'relative', overflow: 'hidden', cursor: 'pointer' }}>
          ...
          <div style={{ position: 'absolute', bottom: 16, left: 16, right: 16, color: '#fff' }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)', marginBottom: 4 }}>
              {p.cat} · {p.year}
            </div>
      ...
      <div style={{ gridColumn: '1 / 3', position: 'relative', overflow: 'hidden', cursor: 'pointer' }}>
        ...
        <div style={{ position: 'absolute', bottom: 20, left: 24, color: '#fff' }}>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)', marginBottom: 4 }}>
            {PROJECTS[5].cat} · {PROJECTS[5].year}
          </div>
    ...
    <div style={{ display: 'flex', justifyContent: 'center', marginTop: 48 }}>
      <a href="Galeria.html" style={{ padding: '16px 36px', background: '#0a0a0a', color: '#fff', fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'JetBrains Mono, monospace', textDecoration: 'none' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
      ...
      <div style={{ padding: '100px 56px', borderRight: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <form style={{ display: 'grid', gap: 0 }}>
          {[
            { l: 'Nombre', p: 'Tu nombre completo' },
            { l: 'Empresa', p: 'Nombre de tu empresa' },
            { l: 'Correo', p: 'correo@empresa.com' },
            { l: 'Teléfono', p: '+52 614 000 0000' },
          ].map(f => (
            <div key={f.l} style={{ borderBottom: '1px solid rgba(255,255,255,0.15)', padding: '18px 0' }}>
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 8 }}>
                {f.l}
              </div>
              <div style={{ fontSize: 17, color: 'rgba(255,255,255,0.35)' }}>{f.p}</div>
            </div>
          ))}
          <div style={{ borderBottom: '1px solid rgba(255,255,255,0.15)', padding: '18px 0' }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 8 }}>
              Sector / Tipo de proyecto
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
              {['Industria', 'Minería', 'Ingeniería', 'Mantenimiento', 'Otro'].map((s, i) => (
                <span key={s} style={{ padding: '6px 12px', border: '1px solid rgba(255,255,255,0.25)', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: 'JetBrains Mono, monospace', background: i === 0 ? 'var(--pp-red)' : 'transparent' }}>{s}</span>
              ))}
            </div>
          </div>
          <div style={{ padding: '18px 0' }}>
            ...
            <div style={{ fontSize: 17, color: 'rgba(255,255,255,0.35)' }}>Cuéntanos sobre tu obra, ubicación y fechas tentativas…</div>
          </div>

          <div style={{ display: 'inline-flex', padding: '18px 36px', background: 'var(--pp-red)', color: '#fff', fontSize: 14, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 24, width: 'fit-content' }}>
            Enviar mensaje →
          </div>
        </form>
      </div>
      ...
      <div style={{ padding: '100px 56px' }}>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 20 }}>
          Contacto directo
        </div>
...
// ==================== Mobile versions (condensed) ====================
const NosotrosMobile = () => (
  <section style={{ padding: '56px 20px', background: '#0a0a0a', color: '#fff' }}>
    <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--pp-red)', fontFamily: 'JetBrains Mono, monospace', marginBottom: 14 }}>¿Quiénes somos?</div>
    ...
    <p style={{ fontSize: 15, lineHeight: 1.6, color: 'rgba(255,255,255,0.8)', marginTop: 20 }}>
...
const ServiciosMobile = () => {
  ...
  <section id="servicios" style={{ padding: '56px 20px', background: '#fff' }}>
    <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--pp-red)', fontFamily: 'JetBrains Mono, monospace', marginBottom: 12 }}>Servicios</div>
    <h2 style={{ fontFamily: 'Archivo, sans-serif', fontWeight: 800, fontSize: 30, lineHeight: 1.0, letterSpacing: '-0.03em', margin: 0, textTransform: 'uppercase' }}>30 servicios. Tres especialidades.</h2>
...
const GaleriaMobile = () => (
  <section style={{ padding: '56px 20px', background: '#fafaf9' }}>
    <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--pp-red)', fontFamily: 'JetBrains Mono, monospace', marginBottom: 12 }}>Galería</div>
...
const ContactoMobile = () => (
  <section style={{ background: '#0a0a0a', color: '#fff', padding: '56px 20px' }}>
    ...
    <div style={{ display: 'grid', gap: 0, marginTop: 20 }}>
      {['Nombre', 'Empresa', 'Correo', 'Teléfono', 'Mensaje'].map(f => (
        <div key={f} style={{ borderBottom: '1px solid rgba(255,255,255,0.15)', padding: '14px 0' }}>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', marginBottom: 6 }}>{f}</div>
          <div style={{ fontSize: 15, color: 'rgba(255,255,255,0.35)' }}>—</div>
        </div>
      ))}
    </div>
    <div style={{ padding: '14px 24px', background: 'var(--pp-red)', color: '#fff', fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 20, textAlign: 'center' }}>
      Enviar mensaje →
    </div>
...
const WhatsappFloat = ({ scale = 1 }) => (
  <a href="https://wa.me/526141666340" target="_blank" rel="noopener" className="pp-whatsapp" style={{
    position: 'absolute', bottom: 24 * scale, right: 24 * scale,
...
Object.assign(window, {
  LOGO_RED, LOGO_H_RED, HERO_IMGS, BRAND_LOGOS,
